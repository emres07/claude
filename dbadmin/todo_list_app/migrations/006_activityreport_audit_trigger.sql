-- Comprehensive audit trigger for activityreport
CREATE OR REPLACE TRIGGER trg_activityreport_audit
BEFORE INSERT OR UPDATE OR DELETE ON activityreport
FOR EACH ROW
BEGIN
    IF INSERTING THEN
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        NULL;
END trg_activityreport_audit;
/

-- Helper function to track column changes
CREATE OR REPLACE FUNCTION CONCAT_CHANGES(
    p_old activityreport%ROWTYPE,
    p_new activityreport%ROWTYPE
) RETURN CLOB IS
    v_changes CLOB := '';
BEGIN
