-- Comprehensive audit trigger for completionrecord
CREATE OR REPLACE TRIGGER trg_completionrecord_audit
BEFORE INSERT OR UPDATE OR DELETE ON completionrecord
FOR EACH ROW
BEGIN
    IF INSERTING THEN
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        NULL;
END trg_completionrecord_audit;
/

-- Helper function to track column changes
CREATE OR REPLACE FUNCTION CONCAT_CHANGES(
    p_old completionrecord%ROWTYPE,
    p_new completionrecord%ROWTYPE
) RETURN CLOB IS
    v_changes CLOB := '';
BEGIN
