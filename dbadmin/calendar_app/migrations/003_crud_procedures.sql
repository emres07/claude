-- ============================================================
-- VERSION 003: Create CRUD Stored Procedures
-- ============================================================

-- ============================================================
-- CRUD Stored Procedures for USER
-- ============================================================
...
BEGIN
  ...
  VALUES (user_seq.NEXTVAL, p_name, p_description, p_status, SYSTIMESTAMP, SYSTIMESTAMP)
...
CREATE OR REPLACE PROCEDURE sp_user_get (
  p_id IN NUMBER,
...
CREATE OR REPLACE PACKAGE BODY pkg_user_ops IS
  ...
  BEGIN
    sp_user_insert(p_name, p_description, 'ACTIVE', p_id);
  END insert_record;
  ...
  BEGIN
    sp_user_update(p_id, p_name, p_description, NULL);
  END update_record;
...
-- ============================================================
-- CRUD Stored Procedures for TRANSACTION
-- ============================================================

-- CREATE PROCEDURE
CREATE OR REPLACE PROCEDURE sp_transaction_insert (
  p_name IN VARCHAR2,
  p_description IN VARCHAR2,
  p_status IN VARCHAR2 DEFAULT 'ACTIVE',
  p_id OUT NUMBER
...
IS
BEGIN
  INSERT INTO transaction (id, name, description, status, created_at, updated_at)
  VALUES (transaction_seq.NEXTVAL, p_name, p_description, p_status, SYSTIMESTAMP, SYSTIMESTAMP)
  RETURNING id INTO p_id;
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    DBMS_OUTPUT.PUT_LINE('Error inserting record: ' || SQLERRM);
    RAISE;
END sp_transaction_insert;
/

-- READ PROCEDURE (Single Record)
CREATE OR REPLACE PROCEDURE sp_transaction_get (
  p_id IN NUMBER,
  p_cursor OUT SYS_REFCURSOR
)
IS
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Error reading record: ' || SQLERRM);
    RAISE;
END sp_transaction_get;
/

-- READ PROCEDURE (All Records)
CREATE OR REPLACE PROCEDURE sp_transaction_get_all (
  p_cursor OUT SYS_REFCURSOR
)
IS
BEGIN
...
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('Error retrieving records: ' || SQLERRM);
    RAISE;
END sp_transaction_get_all;
/

-- UPDATE PROCEDURE
CREATE OR REPLACE PROCEDURE sp_transaction_update (
  p_id IN NUMBER,
  p_name IN VARCHAR2 DEFAULT NULL,
  p_description IN VARCHAR2 DEFAULT NULL,
  p_status IN VARCHAR2 DEFAULT NULL
...
-- DELETE PROCEDURE
CREATE OR REPLACE PROCEDURE sp_transaction_delete (
  p_id IN NUMBER
)
IS
  v_rows_deleted NUMBER;
...
EXCEPTION
  WHEN OTHERS THEN
    ...
    DBMS_OUTPUT.PUT_LINE('Error deleting record: ' || SQLERRM);
    RAISE;
END sp_transaction_delete;
/

-- ============================================================
-- Package for TRANSACTION Operations
-- ============================================================

CREATE OR REPLACE PACKAGE pkg_transaction_ops IS
  PROCEDURE insert_record(
    p_name IN VARCHAR2,
    p_description IN VARCHAR2,
    p_id OUT NUMBER
...
END pkg_transaction_ops;
/

CREATE OR REPLACE PACKAGE BODY pkg_transaction_ops IS
  ...
  BEGIN
    sp_transaction_insert(p_name, p_description, 'ACTIVE', p_id);
  END insert_record;
  ...
  BEGIN
    sp_transaction_get(p_id, p_cursor);
  END get_record;
  ...
  BEGIN
    sp_transaction_get_all(p_cursor);
  END get_all_records;
  ...
  BEGIN
    sp_transaction_update(p_id, p_name, p_description, NULL);
  END update_record;
